...
import com.utn.jpa1.DTOs.DTODetallePedido;
import com.utn.jpa1.DTOs.DTODetalleRankingProductos;
import com.utn.jpa1.DTOs.DTORankingProductos;
import com.utn.jpa1.entities.*;
...
import static com.utn.jpa1.ennumerations.EstadoPedido.PENDIENTE_PAGO;
...
//            if(usuario.getRol() == Rol.CLIENTE) {
            ...
            for (int i = 0; i < articulos.size(); i++) {
                ...
                ArticuloManufacturadoEntity articulo = articuloManufacturadoRepository.getById(dto.getId_ArticuloManufacturado());
...
//                    detallePedidoRepository.save(detallePedido);
            ...
            //Busco Forma Pago
            pedido.setFormadePago(dtoPedido.getFormadePago());

            //Busco domicilio
            DomicilioEntity domicilio = domicilioRepository.search(dtoPedido.getDomicilioCalle(), dtoPedido.getDomicilioNumero());
...
//    }
    public List<Pedido> searchByCliente(Long clienteId) throws Exception {
        try {
            List<Pedido> pedidos = pedidoRepositorio.searchByCliente(clienteId);
            return pedidos;
        }catch (Exception e) {
            throw new Exception(e.getMessage());
        }
    ...
    public List<Pedido> buscarPedidoCocina() throws Exception{
        try{
            List<Pedido> pedidos = pedidoRepositorio.searchPedidosCocina();
            return null;
        }catch (Exception e) {
            throw new Exception(e.getMessage());
        }
    ...
    public DTORankingProductos rankingProductos(String date1, String date2) throws Exception {
        try{
            List<Pedido> pedidos = pedidoRepositorio.searchPedidosBetweenDates(date1, date2);
...
//
            for (int i = 0; i < articulos.size() ; i++) {
                ArticuloManufacturadoEntity articulo = articuloManufacturadoRepository.getById((long) i+1);
